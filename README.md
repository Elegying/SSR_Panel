...
脚本执行 7 个步骤：运行环境准备 → 下载项目 → 配置信息 → 安装 SSR → 安装 Python 运行时 → 部署面板服务 → 服务器优化 → 健康检查。

### 方式二：非交互式部署（批量脚本 / CI）

```bash
export SSR_ADMIN_USER="Elegy"
export SSR_ADMIN_PASS="J.199326"
export SSR_SHARE_HOST="ssr.ssrvpn.vip"
bash install-all.sh
```
...
| 变量 | 说明 | 示例 |
|------|------|------|
| `SSR_ADMIN_USER` | 面板管理员用户名 | `Elegy` |
| `SSR_ADMIN_PASS` | 面板管理员密码 | `J.199326` |
| `SSR_SHARE_HOST` | 节点分享域名/IP | `ssr.ssrvpn.vip` |
...
2. **默认监听 0.0.0.0:5000** — 建议配合 nginx 反向代理或防火墙限制访问
